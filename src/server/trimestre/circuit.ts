...
export interface EtatCircuit {
  ...
  sections: { code: string; chef: ChefDeSection; nom: string; statut: StatutSection; date: string | null; auteur: string | null }[];
...
export async function etatCircuit(db: PrismaClient, periode: Periode): Promise<EtatCircuit> {
  ...
  const lignes = periodeId
    ? await db.circuitTrimestre.findMany({
        where: { periodeId },
        select: { niveau: true, cle: true, statut: true, motif: true, date: true, auteur: { select: { nom: true } } },
      })
    : [];
  ...
  const arrondissements = (await listerArrondissements(db)).map((a) => {
    ...
    return {
      ...
      motif: l?.statut === "RENVOYE" ? l.motif : null,
    };
  });
  ...
  const sections = SECTIONS_CIRCUIT.map((s) => {
    ...
    return { ...s, statut, date: l ? l.date.toISOString() : null, auteur: l?.auteur?.nom ?? null };
  });
  return {
    periodeId,
    arrondissements,
...
async function ecrire(
  ...
  await transaction(async (tx) => {
    ...
    const data = { statut, motif, auteurId, date: new Date() };
...
export function messageIncomplet(etat: EtatCircuit): string {
  ...
  return `La version définitive n'est pas encore possible (${parties.join(" ; ")}). Vous pouvez produire un aperçu.`;
}
